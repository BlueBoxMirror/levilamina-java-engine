...
jbyteArray toBinary(JNIEnv *env, jobject thisObj){
    CompoundTag tag;
    toNativeCompound (env,thisObj,tag);
...
jobject parseBinary(JNIEnv *env, jclass, jbyteArray jbyteArray_binary){
    jbyte* c_byte_list=env->GetByteArrayElements(jbyteArray_binary,nullptr);
    jsize size=env->GetArrayLength(jbyteArray_binary);
    auto tag=CompoundTag::fromBinaryNbt(std::string((char*)c_byte_list,size));
    if(!tag){
        env->ReleaseByteArrayElements(jbyteArray_binary,c_byte_list,JNI_ABORT);
        env->ThrowNew(env->FindClass("java/lang/IllegalArgumentException"), "Invalid NBT binary data");
        return nullptr;
    }
    return newTagCompound(env, (CompoundTag*)&tag);
}
