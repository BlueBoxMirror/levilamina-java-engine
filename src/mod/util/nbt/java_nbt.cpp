...
jobject newTagCompound(JNIEnv* env, const CompoundTag* compound){
    ...
    for(auto it=compound->begin();it!=compound->end();++it){
        ...
        env->DeleteLocalRef(env->CallObjectMethod(jobject_native_compound,jmethod_NbtCompound_put,jstring_native_key,jobject_native_note));
...
jobject newTagList(JNIEnv* env, const ListTag* list){
    ...
    for(auto it=list->begin();it!=list->end();++it){
        ...
        env->DeleteLocalRef(env->CallObjectMethod(jobject_native_list,jmethod_NbtList_add,jobject_native_note));
        env->DeleteLocalRef(jobject_native_note);
    }
...
void toNativeString(JNIEnv *env, jobject obj, StringTag &tag){
    ...
    jstring jstrN_value=(jstring)env->GetObjectField(obj,jfield_NbtString_value);
    const char* c_str_value=env->GetStringUTFChars(jstrN_value,nullptr);
    tag=StringTag(std::string(c_str_value));
    env->ReleaseStringUTFChars(jstrN_value,c_str_value);
    env->DeleteLocalRef(jstrN_value);
}
